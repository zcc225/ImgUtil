 ...
 *    
 *  @since     ： 1.0        创建时间:    2012-12-28        上午11:15:03
 *     
 *  @function： 图片工具类
 *  
 *      
...
public class PictureUtil {
    ...
    public static String imageFormat(String imgPath)throws Exception{
        ...
        MemoryCacheImageInputStream mcis = new MemoryCacheImageInputStream(new ByteArrayInputStream(bt));
        Iterator<ImageReader> it = ImageIO.getImageReaders(mcis);
        while(it.hasNext()){
            ImageReader imgReader = (ImageReader)it.next();
            if(imgReader instanceof GIFImageReader){
                format="gif";
            }else if(imgReader instanceof JPEGImageReader){
                format="jpeg";
            }else if(imgReader instanceof PNGImageReader){
                format="png";
            }else if(imgReader instanceof BMPImageReader){
                format="bmp";
            }
        }
        return format;
